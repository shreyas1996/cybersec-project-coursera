...
ConfigurationManager.prototype.override = function(){
    ...
    if( !fs.existsSync(path.resolve(__dirname, override))){
    ...
    this.config.set('app.override', path.resolve(__dirname, override));
    this.config.loadFile(this.config.get('app.override'));
};
ConfigurationManager.prototype.registerCustomFormats = () => {
    let r = /^\/(.*)\/([gi]{0,2})$/;
    convict.addFormat({
        name: 'CustomRegExp',
